...
state = {'space': [310,230], 'status' : 0, 'level': 30, 'multiplier':10, 'direction': [1,0], 'snake': [0,0], 'food': [200,60], 'counter': 0, 'food_counter': 0, 'difficulty': 1 }
...
def update_screen():
    ...
    screen.text('You have eaten ' + str(state['food_counter']) + ' pieces of food', font_size=10, xy=(20, 10), align='topleft', max_width=280, color='black')
...
def loop():
    ...
    if state['status'] == 0:
        ...
        if state['counter'] < state['level']:
            state['counter'] += state['difficulty']
        else:
            make_move()
            state['counter'] = 0
            update_screen()
